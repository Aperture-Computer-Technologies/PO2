...
#    endif
#    if _MSC_VER >= 1800
#        define PLF_VARIADICS_SUPPORT  // Variadics, in this context, means both variadic templates and variadic macros
                                       // are supported
...
#            endif
#        endif
#    elif defined(__GLIBCXX__)  // Using another compiler type with libstdc++ - we are assuming full c++11 compliance
                                // for compiler - which may not be true
...
#    elif defined(_LIBCPP_CXX03_LANG) \
        || defined(_LIBCPP_HAS_NO_RVALUE_REFERENCES)  // Special case for checking C++11 support with libCPP
#        define PLF_STATIC_ASSERT(check, message) assert(check)
#        define PLF_NOEXCEPT throw()
...
#    if __cplusplus > 201703L                                                                        \
        && ((defined(__clang__) && (__clang_major__ >= 10)) || (defined(__GNUC__) && __GNUC__ >= 10) \
...
namespace LP {  // MASSIVEATOMS addition
    template <typename K, typename V, class Allocator>
    class naive_faster_colony_iter;
...
namespace plf {
    ...
    template <class element_type, class allocator_type = std::allocator<element_type>,
              plf::colony_priority priority = plf::performance>
    class colony : private allocator_type  // Empty base class optimisation (EBCO) - inheriting allocator functions
    {
        ...
        typedef typename choose<priority == plf::performance, unsigned short, unsigned char>::type
            skipfield_type;  // Note: unsigned short is equivalent to uint_least16_t ie. Using 16-bit unsigned integer
                             // in best-case scenario, greater-than-16-bit unsigned integer where platform doesn't
                             // support 16-bit types. unsigned char is always == 1 byte, as opposed to uint_8, which may
                             // not be
      ...
      public:
        ...
        typedef typename std::aligned_storage<sizeof(element_type),
                                              (sizeof(element_type) >= (sizeof(skipfield_type) * 2)
                                               || alignof(element_type) >= (sizeof(skipfield_type) * 2))
                                                  ? alignof(element_type)
                                                  : (sizeof(skipfield_type) * 2)>::type aligned_element_type;
#else
        ...
        // MASSIVEATOMS addition
        template <typename K, typename V, class Allocator>
        friend class LP::naive_faster_colony_iter;

      private:
#ifdef PLF_ALIGNMENT_SUPPORT
        struct alignas(alignof(aligned_element_type)) aligned_allocation_struct {
            char
                data[alignof(aligned_element_type)];  // Using char as sizeof is always guaranteed to be 1 byte
                                                      // regardless of the number of bits in a byte on given computer,
                                                      // whereas for example, uint8_t would fail on machines where there
                                                      // are more than 8 bits in a byte eg. Texas Instruments C54x DSPs.
        };

#    define PLF_GROUP_ALIGNED_BLOCK_SIZE(elements_per_group)                                                     \
        ((((elements_per_group                                                                                   \
            * (((sizeof(aligned_element_type) >= alignof(aligned_element_type)) ? sizeof(aligned_element_type)   \
                                                                                : alignof(aligned_element_type)) \
               + sizeof(skipfield_type)))                                                                        \
           + sizeof(skipfield_type))                                                                             \
          + sizeof(aligned_allocation_struct) - 1)                                                               \
         / sizeof(aligned_allocation_struct))  // The size of a groups' memory block when expressed in multiples of the
                                               // value_type's alignment. We also check to see if alignment is larger
                                               // than sizeof value_type and use alignment size if so.
#else
...
#    define PLF_GROUP_ALIGNED_BLOCK_SIZE(elements_per_group)                            \
        ((elements_per_group * (sizeof(aligned_element_type) + sizeof(skipfield_type))) \
         + sizeof(skipfield_type))  // The size of a groups' memory block when expressed in bytes, since no alignment
                                    // available
#endif
...
#ifdef PLF_ALLOCATOR_TRAITS_SUPPORT
        typedef typename std::allocator_traits<allocator_type>::template rebind_alloc<aligned_element_type>
        ...
        typedef typename std::allocator_traits<allocator_type>::template rebind_alloc<aligned_allocation_struct>
        ...
        typedef typename std::allocator_traits<aligned_element_allocator_type>::pointer
            aligned_pointer_type;  // pointer to the overaligned element type, not the original element type
...
#else
        typedef typename allocator_type::template rebind<aligned_element_type>::other
        ...
        struct group : private aligned_struct_allocator_type  // ebco - inherit allocator functions
        {
            aligned_pointer_type
                last_endpoint;  // The address that is one past the highest cell number that's been used so far in this
                                // group - does not change with erase command but may change with insert (if no
                                // previously-erased locations are available) - is necessary because an iterator cannot
                                // access the colony's end_iterator. Most-used variable in colony use (operator ++, --)
                                // so first in struct. If the colony has been completely filled at some point, it will
                                // be == reinterpret_cast<aligned_pointer_type>(skipfield)
            ...
            const skipfield_pointer_type
                skipfield;  // Skipfield storage. The element and skipfield arrays are allocated contiguously in this
                            // implementation, hence the skipfield pointer also functions as a 'one-past-end' pointer
                            // for the elements array. There will always be one additional skipfield node allocated
                            // compared to the number of elements. This is to ensure a faster ++ iterator operation
                            // (fewer checks are required when this is present). The extra node is unused and always
                            // zero, but checked, and not having it will result in out-of-bounds memory errors.
            group_pointer_type
                previous_group;  // previous group in the linked list of all groups. NULL if no preceding group
            skipfield_type free_list_head;  // The index of the last erased element in the group. The last erased
                                            // element will, in turn, contain the number of the index of the next erased
                                            // element, and so on. If this is == maximum skipfield_type value then
                                            // free_list is empty ie. no erasures have occurred in the group (or if they
                                            // have, the erased locations have subsequently been reused via insert()).
            const skipfield_type
                capacity;  // The element capacity of this particular group - can also be calculated from
                           // reinterpret_cast<aligned_pointer_type>(group->skipfield) - group->elements, however this
                           // space is effectively free due to struct padding and the default sizeof skipfield_type, and
                           // calculating it once is cheaper
            skipfield_type
                size;  // indicates total number of active elements in group - changes with insert and erase commands -
                       // used to check for empty group in erase function, as an indication to remove the group
            group_pointer_type erasures_list_next_group;  // The next group in the singly-linked list of groups with
                                                          // erasures ie. with active erased-element free lists
            size_type group_number;  // Used for comparison (> < >= <= <=>) iterator operators (used by distance
                                     // function and user)
            ...
            group(const skipfield_type elements_per_group, group_pointer_type const previous)
                : last_endpoint(reinterpret_cast<aligned_pointer_type>(
                    PLF_ALLOCATE(aligned_struct_allocator_type, *this, PLF_GROUP_ALIGNED_BLOCK_SIZE(elements_per_group),
            ...
            {
                // Static casts to unsigned int from short not necessary as C++ automatically promotes lesser types for
                // arithmetic purposes.
                std::memset(&*skipfield,
                            0,
                            sizeof(skipfield_type)
                                * (static_cast<size_type>(elements_per_group)
                                   + 1u));  // &* to avoid problems with non-trivial pointers
            }

#else
                                     // This is a hack around the fact that allocator_type::construct only supports copy
                                     // construction in C++03 and copy elision does not occur on the vast majority of
                                     // compilers in this circumstance. So to avoid running out of memory (and losing
                                     // performance) from allocating the same block twice, we're allocating in the
                                     // 'copy' constructor.
            ...
            // Not a real copy constructor ie. actually a constructor. Only used for allocator.construct in C++03 for
            // reasons stated above:
            group(const group &source)
                : aligned_struct_allocator_type(source),
                  last_endpoint(reinterpret_cast<aligned_pointer_type>(
                      PLF_ALLOCATE(aligned_struct_allocator_type, *this, PLF_GROUP_ALIGNED_BLOCK_SIZE(source.capacity),
                       ...
                       const size_type group_num) PLF_NOEXCEPT
            {
                ...
                std::memset(
                    ...
                    0,
                    sizeof(skipfield_type)
                        * static_cast<size_type>(capacity));  // capacity + 1 is not necessary here as the end skipfield
                                                              // is never written to after initialization
            }

            ~group() PLF_NOEXCEPT
            {
                // Null check not necessary (for copied group as above) as delete will also perform a null check.
                PLF_DEALLOCATE(aligned_struct_allocator_type,
                               *this,
                               reinterpret_cast<aligned_struct_pointer_type>(elements),
                               PLF_GROUP_ALIGNED_BLOCK_SIZE(capacity));
            }
        };
      ...
      public:
        ...
        class colony_iterator {
          ...
          public:
            ...
            typedef
                typename choose<is_const, typename colony::const_reference, typename colony::reference>::type reference;
...
#endif
            {
                ...
                if ((element_pointer += static_cast<size_type>(skip) + 1u) == group_pointer->last_endpoint
                    && group_pointer->next_group
                           != NULL)  // ie. beyond end of current memory block. Second condition allows iterator to
                                     // reach end(), which may be 1 past end of block, if block has been fully used and
            ...
            colony_iterator &operator--()
            {
                ...
                if (element_pointer != group_pointer->elements)  // ie. not already at beginning of group
                {
                    ...
                    if ((element_pointer -= static_cast<size_type>(skip) + 1u)
                        != group_pointer->elements - 1)  // ie. iterator was not already at beginning of colony (with
                                                         // some previous consecutive deleted elements), and skipfield
                                                         // does not takes us into the previous group)
                    {
                        return *this;
                    }
                }
          ...
          public:
            ...
            friend inline typename colony_iterator::difference_type distance(const colony_iterator &first,
          ...
          private:
            ...
            void advance(
                difference_type distance)  // Cannot be noexcept due to the possibility of an uninitialized iterator
            {
                ...
                {
                    // Code explanation:
                    // For the initial state of the iterator, we don't know how what elements have been erased before
                    // that element in that group. So for the first group, we follow the following logic:
                    // 1. If no elements have been erased in the group, we do simple addition to progress either to
                    // within the group (if the distance is small enough) or the end of the group and subtract from
                    // distance accordingly.
                    // 2. If any of the first group elements have been erased, we manually iterate, as we don't know
                    // whether the erased elements occur before or after the initial iterator position, and we subtract
                    // 1 from the distance amount each time. Iteration continues until either distance becomes zero, or
                    // we reach the end of the group.

                    // For all subsequent groups, we follow this logic:
                    // 1. If distance is larger than the total number of non-erased elements in a group, we skip that
                    // group and subtract the number of elements in that group from distance
                    // 2. If distance is smaller than the total number of non-erased elements in a group, then:
                    //	  a. if there're no erased elements in the group we simply add distance to group->elements to
                    //find the new location for the iterator 	  b. if there are erased elements in the group, we manually
                    //iterate and subtract 1 from distance on each iteration, until the new iterator location is found
                    //ie. distance = 0
                    ...
                    // Special case for initial element pointer and initial group (we don't know how far into the group
                               ...
                               + *(group_pointer->skipfield))  // ie. != first non-erased element in group
                    {
                        const difference_type distance_from_end
                            = static_cast<difference_type>(group_pointer->last_endpoint - element_pointer);

                        if (group_pointer->size
                            == static_cast<skipfield_type>(
                                distance_from_end))  // ie. if there are no erasures in the group (using endpoint -
                                                     // elements_start to determine number of elements in group just in
                                                     // case this is the last group of the colony, in which case
                                                     // group->last_endpoint != group->elements + group->capacity)
                        {
                    ...
                    // Intermediary groups - at the start of this code block and the subsequent block, the position of
                    // the iterator is assumed to be the first non-erased element in the current group:
                    while (static_cast<difference_type>(group_pointer->size) <= distance) {
                            ...
                            == NULL)  // either we've reached end() or gone beyond it, so bound to end()
                        {
                            element_pointer = group_pointer->last_endpoint;
                            skipfield_pointer
                                = group_pointer->skipfield + (group_pointer->last_endpoint - group_pointer->elements);
                            return;
                        }
                    ...
                    if (group_pointer->free_list_head
                        == std::numeric_limits<skipfield_type>::max())  // No erasures in this group, use straight
                                                                        // pointer addition
                    {
                ...
                else if (distance < 0)  // for negative change
                {
                    ...
                    {
                        if (group_pointer->free_list_head
                            == std::numeric_limits<skipfield_type>::max())  // ie. no prior erasures have occurred in
                                                                            // this group
                        {
                            ...
                            if (group_pointer->previous_group == NULL) {
                                element_pointer
                                    = group_pointer->elements
                                      + *(group_pointer->skipfield);  // This is first group, so bound to begin() (just
                                                                      // in case final decrement took us before begin())
                                skipfield_pointer = group_pointer->skipfield + *(group_pointer->skipfield);
                                return;
                            }
                        }
                    ...
                    // Intermediary groups - at the start of this code block and the subsequent block, the position of
                    // the iterator is assumed to be either the first non-erased element in the next group over, or
                    // end():
                    while (static_cast<difference_type>(group_pointer->size) < distance) {
            ...
            difference_type distance(const colony_iterator &last) const
            {
                ...
                // Otherwise, find which iterator is later in colony, copy that to iterator2. Copy the lower to
                // iterator1. If they are not pointing to elements in the same group, process the intermediate groups
                // and add distances, skipping manual incrementation in all but the initial and final groups. In the
                // initial and final groups, manual incrementation must be used to calculate distance, if there have
                // been no prior erasures in those groups. If there are no prior erasures in either of those groups, we
                // can use pointer arithmetic to calculate the distances for those groups.
                    ...
                    != iterator2.group_pointer)  // if not in same group, process intermediate groups
                {
                    ...
                    if (iterator1.group_pointer->free_list_head
                        == std::numeric_limits<skipfield_type>::max())  // If no prior erasures have occured in this
                                                                        // group we can do simple addition
                    {
                        distance += static_cast<difference_type>(iterator1.group_pointer->last_endpoint
                ...
                if (iterator2.group_pointer->free_list_head
                    == std::numeric_limits<skipfield_type>::max())  // ie. no erasures in this group, direct subtraction
                                                                    // is possible
                {
                    distance += iterator2.skipfield_pointer - iterator1.skipfield_pointer;
                }
                else if (iterator2.group_pointer->last_endpoint - 1 >= iterator2.element_pointer
                         || iterator2.element_pointer + *(iterator2.skipfield_pointer + 1)
                                == iterator2.group_pointer
                                       ->last_endpoint)  // ie. if iterator2 is .end() or the last element in the block
                {
                    distance += static_cast<difference_type>(iterator2.group_pointer->size)
                                - (iterator2.group_pointer->last_endpoint - iterator2.element_pointer);
                }
                else {
        ...
        class colony_reverse_iterator {
          ...
          public:
            ...
            typedef typename choose<r_is_const, typename colony::const_reference, typename colony::reference>::type
                reference;
            ...
            // In this case we have to redefine the algorithm, rather than using the internal iterator's -- operator, in
            // order for the reverse_iterator to be allowed to reach rend() ie. begin_iterator - 1
            colony_reverse_iterator &operator++()
            {
                ...
                else  // necessary so that reverse_iterator can end up == rend(), if we were already at first element in
                      // colony
                {
                    --element_pointer;
                    --skipfield_pointer;
            ...
            inline typename colony::iterator base() const { return ++(typename colony::iterator(it)); }
            ...
            colony_reverse_iterator(const colony_reverse_iterator<!r_is_const> &source) PLF_NOEXCEPT : it(source.it) {}

            template <bool is_const>
            explicit colony_reverse_iterator(const colony_iterator<is_const> &source) PLF_NOEXCEPT : it(source)
            {
            }
          ...
          public:
            ...
            friend inline colony_reverse_iterator next(const colony_reverse_iterator &it,
            ...
            template <class distance_type>
            friend inline colony_reverse_iterator prev(const colony_reverse_iterator &it,
                                                       const difference_type distance)
            {
                colony_reverse_iterator return_iterator(it);
                return_iterator.advance(-(static_cast<difference_type>(distance)));
            ...
            friend inline typename colony_reverse_iterator::difference_type distance(
                const colony_reverse_iterator &first, const colony_reverse_iterator &last)
            {
                return first.distance(last);
            }
            ...
            inline difference_type distance(const colony_reverse_iterator &last) const { return last.it.distance(it); }

          private:
            // Advance for reverse_iterator and const_reverse_iterator - this needs to be implemented slightly
            // differently to forward-iterator's advance, as it needs to be able to reach rend() (ie. begin() - 1) and
            // to be bounded by rbegin():
            void advance(difference_type distance)  // could cause exception if iterator is uninitialized
            {
                ...
                if (distance > 0) {
                    ...
                    // Special case for initial element pointer and initial group (we don't know how far into the group
                    // the element pointer is) Since a reverse_iterator cannot == last_endpoint (ie. before rbegin()) we
                    // don't need to check for that like with iterator
                    ...
                    // Intermediary groups - at the start of this code block and the subsequent block, the position of
                    // the iterator is assumed to be the first non-erased element in the next group:
                    while (static_cast<difference_type>(group_pointer->size) < distance) {
                    ...
                    assert(!((element_pointer
                              == (group_pointer->last_endpoint - 1)
                                     - *(group_pointer->skipfield
                                         + (group_pointer->last_endpoint - group_pointer->elements) - 1))
                               ...
                               + *(group_pointer->skipfield))  // ie. != first non-erased element in group
                    {
                            ...
                            == std::numeric_limits<skipfield_type>::max())  // ie. if there are no erasures in the group
                        {
                            ...
                            const skipfield_pointer_type endpoint
                                = skipfield_pointer + (group_pointer->last_endpoint - element_pointer);
                    ...
                    // Intermediary groups - at the start of this code block and the subsequent block, the position of
                    // the iterator is assumed to be the first non-erased element in the current group, as a result of
      ...
      private:
        ...
        group_pointer_type groups_with_erasures_list_head,  // Head of the singly-linked list of groups which have
        ...
        struct ebco_pair2 : tuple_allocator_type  // Packaging the element pointer allocator with a lesser-used member
                                                  // variable, for empty-base-class optimisation
        {
...
#define PLF_MIN_BLOCK_CAPACITY                                                                   \
    (sizeof(aligned_element_type) * 8 > (sizeof(plf::colony<element_type>) + sizeof(group)) * 2) \
        ? 8                                                                                      \
        : (((sizeof(plf::colony<element_type>) + sizeof(group)) * 2) / sizeof(aligned_element_type))

        inline void check_capacities_conformance(plf::colony_limits capacities) const
        {
            if (capacities.min < 2 || capacities.min > capacities.max
                || capacities.max > std::numeric_limits<skipfield_type>::max()) {
        ...
        inline void check_skipfield_conformance() const PLF_NOEXCEPT
        {
            PLF_STATIC_ASSERT(
                sizeof(element_type) >= sizeof(skipfield_type) * 2,
                "Element type is not large enough to accomodate colony requirements under C++98/03. Change to C++11 or "
                "above, or use a larger type.");  // eg. under C++98/03, aligned_storage is not available, so
                                                  // sizeof(skipfield type) * 2 must be larger or equal to
                                                  // sizeof(element_type), otherwise the doubly-linked free lists of
                                                  // erased element indexes will not work correctly. So if you're
                                                  // storing chars, for example, and the skipfield type is unsigned
                                                  // short (currently default for this implementation), the compiler
                                                  // will flag you with this assert. Which means you cannot store char
                                                  // or unsigned char in colony under C++03, and if storing short or
                                                  // unsigned short you must use the priority::memory_use parameter in
                                                  // your template instantiation. Or just use C++11 and above.
        }
#endif
      ...
      public:
        ...
        colony(const colony &source)
            : allocator_type(source),
              ...
              total_size(0),
              total_capacity(0),
              tuple_allocator_pair(static_cast<skipfield_type>(
                  (source.tuple_allocator_pair.min_group_capacity > source.total_size)
                      ? source.tuple_allocator_pair.min_group_capacity
                      : ((source.total_size > source.group_allocator_pair.max_group_capacity)
                             ? source.group_allocator_pair.max_group_capacity
                             : source.total_size))),  // min group size is set to value closest to total number of
                                                      // elements in source colony in order to not create unnecessary
                                                      // small groups in the range-insert below, then reverts to the
                                                      // original min group size afterwards. This effectively saves a
                                                      // call to reserve.
              group_allocator_pair(source.group_allocator_pair.max_group_capacity)
        {  // can skip checking for skipfield conformance here as the skipfields must be equal between the destination
           // and source, and source will have already had theirs checked. Same applies for other copy and move
           // constructors below
            range_assign(source.begin_iterator, source.total_size);
            tuple_allocator_pair.min_group_capacity
                = source.tuple_allocator_pair
                      .min_group_capacity;  // reset to correct value for future clear() or erasures
        }
      ...
      private:
        ...
        {
#ifdef PLF_TYPE_TRAITS_SUPPORT
            if PLF_CONSTEXPR (std::is_trivial<group_pointer_type>::value && std::is_trivial<aligned_pointer_type>::value
                              && std::is_trivial<skipfield_pointer_type>::value)  // if all pointer types are trivial,
                                                                                  // we can just nuke it from orbit with
      ...
      public:
        ...
        colony(const size_type fill_number, const element_type &element,
               const plf::colony_limits capacities
               = plf::colony_limits(PLF_MIN_BLOCK_CAPACITY, std::numeric_limits<skipfield_type>::max()),
...
#    endif
        ...
        inline PLF_FORCE_INLINE iterator begin() PLF_NOEXCEPT { return begin_iterator; }

        inline PLF_FORCE_INLINE const_iterator begin() const PLF_NOEXCEPT { return begin_iterator; }

        inline PLF_FORCE_INLINE iterator end() PLF_NOEXCEPT { return end_iterator; }

        inline PLF_FORCE_INLINE const_iterator end() const PLF_NOEXCEPT { return end_iterator; }

        inline PLF_FORCE_INLINE const_iterator cbegin() const PLF_NOEXCEPT { return begin_iterator; }

        inline PLF_FORCE_INLINE const_iterator cend() const PLF_NOEXCEPT { return end_iterator; }

        inline reverse_iterator rbegin() PLF_NOEXCEPT
        {
            return (end_iterator.group_pointer != NULL) ? ++reverse_iterator(end_iterator)
        ...
        {
            return (end_iterator.group_pointer != NULL) ? ++const_reverse_iterator(end_iterator)
...
#else
                PLF_CONSTRUCT(
                    group_allocator_type, group_allocator_pair, new_group, group(elements_per_group, previous));
#endif
            }
            catch (...) {
...
#endif  // If compiler doesn't support traits, iterate regardless - trivial destructors will not be called, hopefully
        // compiler will optimise the 'destruct' loop out for POD types
                    if (total_size != 0) {
                        while (true)  // Erase elements without bothering to update skipfield - much faster:
                        {
                            ...
                            do {
                                PLF_DESTROY(
                                    allocator_type, *this, reinterpret_cast<pointer>(begin_iterator.element_pointer));
                            ...
                            } while (begin_iterator.element_pointer
                                     != end_pointer);  // ie. beyond end of available data
            ...
            *(new_location.skipfield_pointer) = 0;
            ...
            if (new_location.group_pointer == begin_iterator.group_pointer
                && new_location.element_pointer
                       < begin_iterator
                             .element_pointer) { /* ie. begin_iterator was moved forwards as the result of an erasure at
                                                    some point, this erased element is before the current begin, hence,
                                                    set current begin iterator to this element */
      ...
      public:
        iterator insert(const element_type &element)
        {
            if (end_iterator.element_pointer != NULL) {
                switch (((groups_with_erasures_list_head != NULL) << 1)
                    ...
                    case 0:  // ie. there are no erased elements and end_iterator is not at end of current final group
                    {
...
#ifdef PLF_TYPE_TRAITS_SUPPORT
                        if PLF_CONSTEXPR (std::is_nothrow_copy_constructible<element_type>::
                                              value) {  // For no good reason this compiles to ridiculously faster code
                                                        // under GCC 5-9 in raw small struct tests with large N:
                            PLF_CONSTRUCT(allocator_type,
                                          *this,
...
#endif
                        {
                            ...
                            end_iterator.group_pointer->last_endpoint
                                = ++end_iterator.element_pointer;  // Shift the addition to the second operation,
                                                                   // avoiding a try-catch block if an exception is
                    ...
                    case 1:  // ie. there are no erased elements and end_iterator is at end of current final group - ie.
                             // colony is full - create new group
                    {
                        group_pointer_type next_group;

                        if (unused_groups_head == NULL) {
                            const skipfield_type new_group_size
                                = (total_size < static_cast<size_type>(group_allocator_pair.max_group_capacity))
                                      ? static_cast<skipfield_type>(total_size)
                                      : group_allocator_pair.max_group_capacity;
                            ...
                            if PLF_CONSTEXPR (std::is_nothrow_copy_constructible<element_type>::value) {
                                PLF_CONSTRUCT(
                                    allocator_type, *this, reinterpret_cast<pointer>(next_group->elements), element);
                            }
                            else
#endif
                            {
                                try {
                                    PLF_CONSTRUCT(allocator_type,
                                                  *this,
                                                  reinterpret_cast<pointer>(next_group->elements),
                                                  element);
                                }
                                catch (...) {
                            ...
                            next_group->reset(
                                1, NULL, end_iterator.group_pointer, end_iterator.group_pointer->group_number + 1u);
                        }
                    ...
                    default:  // ie. there are erased elements, reuse previous-erased element locations
                    {
                        iterator new_location(
                            ...
                            groups_with_erasures_list_head->elements + groups_with_erasures_list_head->free_list_head,
                            groups_with_erasures_list_head->skipfield + groups_with_erasures_list_head->free_list_head);

                        // We always reuse the element at the start of the skipblock, this is also where the free-list
                        // information for that skipblock is stored. Get the previous free-list node's index from this
                        // memory space, before we write to our element to it. 'Next' index is always the free_list_head
                        // (as represented by the maximum value of the skipfield type) here so we don't need to get it:
                        const skipfield_type prev_free_list_index
                            = *(reinterpret_cast<skipfield_pointer_type>(new_location.element_pointer));
                        PLF_CONSTRUCT(
                            allocator_type, *this, reinterpret_cast<pointer>(new_location.element_pointer), element);
            ...
            else  // ie. newly-constructed colony, no insertions yet and no groups
            {
                ...
                if PLF_CONSTEXPR (std::is_nothrow_copy_constructible<element_type>::value) {
                    PLF_CONSTRUCT(
                        allocator_type, *this, reinterpret_cast<pointer>(end_iterator.element_pointer++), element);
                }
                else
#endif
                {
...
#ifdef PLF_MOVE_SEMANTICS_SUPPORT
        iterator insert(
            element_type &&element)  // The move-insert function is near-identical to the regular insert function, with
                                     // the exception of the element construction method and is_nothrow tests.
        {
            if (end_iterator.element_pointer != NULL) {
                switch (((groups_with_erasures_list_head != NULL) << 1)
                        | (end_iterator.element_pointer
...
#    endif
        ...
        iterator emplace(arguments &&...parameters)  // The emplace function is near-identical to the regular insert
                                                     // function, with the exception of the element construction method,
                                                     // and change to is_nothrow tests.
        {
            if (end_iterator.element_pointer != NULL) {
                switch (((groups_with_erasures_list_head != NULL) << 1)
                        | (end_iterator.element_pointer
...
#    endif
      ...
      private:
        ...
        {
#ifdef PLF_TYPE_TRAITS_SUPPORT  // try to ensure this code will not be generated if this function is not called in
                                // fill() or range_fill()
            if PLF_CONSTEXPR (!std::is_nothrow_copy_constructible<element_type>::value)
#endif
            {
                ...
                const skipfield_type elements_constructed_before_exception
                    = static_cast<skipfield_type>(end_iterator.element_pointer - end_iterator.group_pointer->elements);
            ...
            if PLF_CONSTEXPR (std::is_nothrow_copy_constructible<element_type>::value) {
                if PLF_CONSTEXPR (std::is_trivially_copyable<element_type>::value
                                  && std::is_trivially_copy_constructible<
                                      element_type>::value)  // ie. we can get away with using the cheaper fill_n here
                                                             // if there is no chance of an exception being thrown:
                {
#    ifdef PLF_ALIGNMENT_SUPPORT
                    if PLF_CONSTEXPR (sizeof(aligned_element_type) != sizeof(element_type)) {
                        alignas(alignof(aligned_element_type)) element_type aligned_copy
                            = element;  // to avoid potentially violating memory boundaries in line below, create an
                                        // initial copy object of same (but aligned) type
                        std::fill_n(end_iterator.element_pointer,
                                    size,
...
#    endif
        ...
        void recover_from_partial_skipblock_fill(aligned_pointer_type const location,
        ...
        {
#ifdef PLF_TYPE_TRAITS_SUPPORT  // try to ensure this code will not be generated if this function is not called in
                                // fill_skipblock or range_fill_skipblock
            if PLF_CONSTEXPR (!std::is_nothrow_copy_constructible<element_type>::value)
#endif
            {
                // Reconstruct existing skipblock and free-list indexes to reflect partially-reused skipblock:
                const skipfield_type elements_constructed_before_exception
                    = static_cast<skipfield_type>((current_location - 1) - location);
                groups_with_erasures_list_head->size = static_cast<skipfield_type>(
                    groups_with_erasures_list_head->size + elements_constructed_before_exception);
                total_size += elements_constructed_before_exception;
        ...
        void fill_skipblock(const element_type &element, aligned_pointer_type const location,
                            skipfield_pointer_type const skipfield_pointer, const skipfield_type size)
        {
#ifdef PLF_TYPE_TRAITS_SUPPORT
            if PLF_CONSTEXPR (std::is_nothrow_copy_constructible<element_type>::value) {
                if PLF_CONSTEXPR (std::is_trivially_copyable<element_type>::value
...
#    endif
                ...
                for (aligned_pointer_type current_location = location; current_location != fill_end;
                    ...
                    try {
                        ...
                        recover_from_partial_skipblock_fill(
                            location, current_location, skipfield_pointer, prev_free_list_node);
                        throw;
                    }
                }
            }
            ...
            std::memset(
                skipfield_pointer, 0, size * sizeof(skipfield_type));  // reset skipfield nodes within skipblock to 0
            groups_with_erasures_list_head->size
                = static_cast<skipfield_type>(groups_with_erasures_list_head->size + size);
            total_size += size;
        }

        void fill_unused_groups(size_type size, const element_type &element, size_type group_number,
                                group_pointer_type previous_group, const group_pointer_type current_group)
        {
            end_iterator.group_pointer = current_group;

            for (; end_iterator.group_pointer->capacity < size;
      ...
      public:
        ...
        void insert(size_type size, const element_type &element)
        {
            ...
            while (groups_with_erasures_list_head != NULL)  // skipblock loop: breaks when colony is exhausted of
                                                            // reusable skipblocks, or returns if size == 0
            {
                ...
                if (skipblock_size <= size) {
                    ...
                    if (groups_with_erasures_list_head->free_list_head
                        != std::numeric_limits<skipfield_type>::max())  // ie. there are more skipblocks to be filled in
                                                                        // this group
                    {
                        *(reinterpret_cast<skipfield_pointer_type>(groups_with_erasures_list_head->elements
                                                                   + groups_with_erasures_list_head->free_list_head)
                          + 1)
                            = std::numeric_limits<skipfield_type>::max();  // set 'next' index of new free list head to
                                                                           // 'end' (numeric max)
                    }
                    else {
                        groups_with_erasures_list_head
                ...
                else  // skipblock is larger than remaining number of elements
                {
                    ...
                    *(skipfield_pointer + skipblock_size - 1) = new_skipblock_size;
                    groups_with_erasures_list_head->free_list_head = static_cast<skipfield_type>(
                        groups_with_erasures_list_head->free_list_head + size);  // set free list head to new start node

                    // Update free list with new head:
                    *(reinterpret_cast<skipfield_pointer_type>(element_pointer + size)) = prev_index;
                    *(reinterpret_cast<skipfield_pointer_type>(element_pointer + size) + 1)
                        = std::numeric_limits<skipfield_type>::max();
            ...
            // This variable is either the remaining capacity of the group or the number of elements yet to be filled,
            ...
            // Use unused groups:
            end_iterator.group_pointer->next_group = unused_groups_head;
            fill_unused_groups(size,
                               element,
                               end_iterator.group_pointer->group_number + 1,
      ...
      private:
        ...
        iterator_type range_fill(iterator_type it, const skipfield_type size)
        {
            ...
            if PLF_CONSTEXPR (std::is_nothrow_copy_constructible<element_type>::value) {
                ...
                do {
                    PLF_CONSTRUCT(
                        allocator_type, *this, reinterpret_cast<pointer>(end_iterator.element_pointer), *it++);
                } while (++end_iterator.element_pointer != fill_end);
            }
            else
#endif
        ...
        template <class iterator_type>
        iterator_type range_fill_skipblock(iterator_type it, aligned_pointer_type const location,
                                           skipfield_pointer_type const skipfield_pointer, const skipfield_type size)
        {
            const aligned_pointer_type fill_end = location + size;
        ...
        void range_insert(iterator_type it, size_type size)
        {
                                                            ...
                                                            // reusable skipblocks, or returns if size == 0
            {
                ...
                else  // skipblock is larger than remaining number of elements
                {
                    const skipfield_type prev_index = *(reinterpret_cast<skipfield_pointer_type>(
                        element_pointer));  // save before element location is overwritten
                    it = range_fill_skipblock(
                        it, element_pointer, skipfield_pointer, static_cast<skipfield_type>(size));
                    const skipfield_type new_skipblock_size = static_cast<skipfield_type>(skipblock_size - size);
            ...
            range_fill_unused_groups(
                size, it, end_iterator.group_pointer->group_number + 1, end_iterator.group_pointer, unused_groups_head);
        }
      ...
      public:
        ...
        requires(!std::same_as<iterator_type1, iterator_type2> && std::equality_comparable_with<iterator_type1, iterator_type2> && !std::integral<iterator_type1> && !std::integral<iterator_type2>) inline void insert(
            const iterator_type1 first, const iterator_type2 last)
        {
      ...
      public:
        // must return iterator to subsequent non-erased element (or end()), in case the group containing the element
        // which the iterator points to becomes empty after the erasure, and is thereafter removed from the colony
        // chain, making the current iterator invalid and unusable in a ++ operation:
                           ...
                           it)  // if uninitialized/invalid iterator supplied, function could generate an exception
        {
            ...
            if PLF_CONSTEXPR (!std::is_trivially_destructible<element_type>::
                                  value)  // This if-statement should be removed by the compiler on resolution of
                                          // element_type. For some optimizing compilers this step won't be necessary
                                          // (for MSVC 2013 it makes a difference)
#endif
            {
            ...
            if (it.group_pointer->size--
                != 1)  // ie. non-empty group at this point in time, don't consolidate - optimization note: GCC
                       // optimizes postfix - 1 comparison better than prefix - 1 comparison in many cases.
            {
                // Code logic for following section:
                // ---------------------------------
                // If current skipfield node has no skipped node on either side, continue as usual
                // If node only has skipped node on left, set current node and start node of the skipblock to left node
                // value + 1. If node only has skipped node on right, make this node the start node of the skipblock and
                // update end node If node has skipped nodes on left and right, set start node of left skipblock and end
                // node of right skipblock to the values of the left + right nodes + 1

                // Optimization explanation:
                // The contextual logic below is the same as that in the insert() functions but in this case the value
                // of the current skipfield node will always be zero (since it is not yet erased), meaning no additional
                // manipulations are necessary for the previous skipfield node comparison - we only have to check
                // against zero
                const char prev_skipfield
                    = *(it.skipfield_pointer - (it.skipfield_pointer != it.group_pointer->skipfield)) != 0;
                const char after_skipfield
                    = *(it.skipfield_pointer + 1)
                      != 0;  // NOTE: boundary test (checking against end-of-elements) is able to be skipped due to the
                             // extra skipfield node (compared to element field) - which is present to enable faster
                             // iterator operator ++ operations
                ...
                switch ((after_skipfield << 1) | prev_skipfield) {
                    case 0:  // no consecutive erased elements
                    {
                        *it.skipfield_pointer = 1;  // solo skipped node
                        ...
                        if (it.group_pointer->free_list_head
                            != std::numeric_limits<skipfield_type>::max())  // ie. if this group already has some erased
                                                                            // elements
                        {
                            *(reinterpret_cast<skipfield_pointer_type>(it.group_pointer->elements
                                                                       + it.group_pointer->free_list_head)
                              + 1)
                                = index;  // set prev free list head's 'next index' number to the index of the current
                                          // element
                        }
                        else {
                        ...
                        *(reinterpret_cast<skipfield_pointer_type>(it.element_pointer))
                            = it.group_pointer->free_list_head;
                    ...
                    case 2:  // following erased consecutive elements, none preceding
                    {
                        ...
                        *(it.skipfield_pointer + following_value - 1) = *(it.skipfield_pointer) = following_value;
                        ...
                        const skipfield_type following_next
                            = *(reinterpret_cast<skipfield_pointer_type>(it.element_pointer + 1) + 1);
                        ...
                        *(reinterpret_cast<skipfield_pointer_type>(it.element_pointer) + 1) = following_next;
                        ...
                        if (following_previous != std::numeric_limits<skipfield_type>::max()) {
                            *(reinterpret_cast<skipfield_pointer_type>(it.group_pointer->elements + following_previous)
                              + 1)
                        ...
                        if (following_next != std::numeric_limits<skipfield_type>::max()) {
                            *(reinterpret_cast<skipfield_pointer_type>(it.group_pointer->elements + following_next))
                    ...
                    case 3:  // both preceding and following consecutive erased elements - erased element is between two
                             // skipblocks
                    {
                        ...
                        *(it.skipfield_pointer - preceding_value) = *(it.skipfield_pointer + following_value - 1)
                        ...
                        if (following_previous != std::numeric_limits<skipfield_type>::max()) {
                            *(reinterpret_cast<skipfield_pointer_type>(it.group_pointer->elements + following_previous)
                              + 1)
                                = following_next;  // Set next index of previous free list node to this node's 'next'
                                                   // index
                        }

                        if (following_next != std::numeric_limits<skipfield_type>::max()) {
                            *(reinterpret_cast<skipfield_pointer_type>(it.group_pointer->elements + following_next))
                                = following_previous;  // Set previous index of next free list node to this node's
                                                       // 'previous' index
                        }
                        else {
                ...
                if (return_iterator.element_pointer == it.group_pointer->last_endpoint
                ...
                if (it.element_pointer
                    == begin_iterator.element_pointer)  // If original iterator was first element in colony, update it's
                                                        // value with the next non-erased element:
                {
            ...
            switch ((it.group_pointer->next_group != NULL)
                    | ((it.group_pointer != begin_iterator.group_pointer) << 1)) {
                case 0:  // ie. it.group_pointer == begin_iterator.group_pointer && it.group_pointer->next_group ==
                         // NULL; only group in colony
                {
                ...
                case 1:  // ie. group_pointer == begin_iterator.group_pointer && group_pointer->next_group != NULL.
                         // Remove first group, change first group to next group
                {
                    ...
                    if (it.group_pointer->free_list_head
                        != std::numeric_limits<skipfield_type>::max())  // Erasures present within the group, ie. was
                                                                        // part of the linked list of groups with
                                                                        // erasures.
                    {
                    ...
                    // note: end iterator only needs to be changed if the deleted group was the final group in the chain
                    // ie. not in this case
                    begin_iterator.element_pointer
                        = begin_iterator.group_pointer->elements
                          + *(begin_iterator.group_pointer
                                  ->skipfield);  // If the beginning index has been erased (ie. skipfield != 0), skip to
                                                 // next non-erased element
                    begin_iterator.skipfield_pointer
                        = begin_iterator.group_pointer->skipfield + *(begin_iterator.group_pointer->skipfield);

                    return begin_iterator;
                }
                case 3:  // this is a non-first group but not final group in chain: delete the group, then link previous
                ...
                default:  // this is a non-first group and the final group in the chain
                {
                    ...
                    end_iterator.group_pointer
                        = it.group_pointer->previous_group;  // end iterator needs to be changed as element supplied was
                                                             // the back element of the colony
                    end_iterator.element_pointer
                        = reinterpret_cast<aligned_pointer_type>(end_iterator.group_pointer->skipfield);
                    end_iterator.skipfield_pointer
        ...
        iterator erase(
            ...
            const const_iterator iterator2)  // if uninitialized/invalid iterators supplied, function could generate an
                                             // exception. If iterator1 > iterator2, behaviour is undefined.
        {
                ...
                != iterator2.group_pointer)  // ie. if start and end iterators are in separate groups
            {
                if (current.element_pointer
                    != current.group_pointer->elements
                           + *(current.group_pointer->skipfield))  // if iterator1 is not the first non-erased element
                                                                   // in it's group - most common case
                {
                    ...
                    // Schema: first erase all non-erased elements until end of group & remove all skipblocks
                    // post-iterator1 from the free_list. Then, either update preceding skipblock or create new one:

#ifdef PLF_TYPE_TRAITS_SUPPORT  // if trivially-destructible, and C++11 or higher, and no erasures in group, skip while
                                // loop below and just jump straight to the location
...
#endif
                    {
                        while (current.element_pointer != end) {
                            ...
                            {
                                const skipfield_type prev_free_list_index
                                    = *(reinterpret_cast<skipfield_pointer_type>(current.element_pointer));
                                const skipfield_type next_free_list_index
                                    = *(reinterpret_cast<skipfield_pointer_type>(current.element_pointer) + 1);
                                ...
                                if (next_free_list_index == std::numeric_limits<skipfield_type>::max()
                                    && prev_free_list_index
                                           == std::numeric_limits<skipfield_type>::max())  // if this is the last
                                                                                           // skipblock in the free list
                                {
                                    remove_from_groups_with_erasures_list(
                                        iterator1
                                            .group_pointer);  // remove group from list of free-list groups - will be
                                                              // added back in down below, but not worth optimizing for
...
#endif
                                    {
                                        while (current.element_pointer
                                               != end)  // miniloop - avoid checking skipfield for rest of elements in
                                                        // group, as there are no more skipped elements now
                                        {
                                            PLF_DESTROY(allocator_type,
                                                        *this,
                                                        reinterpret_cast<pointer>(
                                                            current.element_pointer++));  // Destruct element
                                        }
                                    }
                                    ...
                                    break;  // end overall while loop
                                         ...
                                         == std::numeric_limits<skipfield_type>::max())  // if this is the head of the
                                                                                         // free list
                                {
                                ...
                                else  // either a tail or middle free list node
                                {
                                    *(reinterpret_cast<skipfield_pointer_type>(current.group_pointer->elements
                                    ...
                                    if (prev_free_list_index
                                        != std::numeric_limits<skipfield_type>::max())  // ie. not the tail free list
                                                                                        // node
                                    {
                                        *(reinterpret_cast<skipfield_pointer_type>(current.group_pointer->elements
                                                                                   + prev_free_list_index)
                        ...
                        *(iterator1.skipfield_pointer + distance_to_end - 1) = distance_to_end;  // set end node value
                                                                            ...
                                                                            // elements
                        {
                            *(reinterpret_cast<skipfield_pointer_type>(iterator1.group_pointer->elements
                                                                       + iterator1.group_pointer->free_list_head)
                              + 1)
                                = index;  // set prev free list head's 'next index' number to the index of the iterator1
                                          // element
                        }
                        else {
                    ...
                    iterator1.group_pointer->size
                        = static_cast<skipfield_type>(iterator1.group_pointer->size - number_of_group_erasures);
                ...
                if (previous_group != NULL) {
                    ...
                    begin_iterator
                        = iterator2;  // This line is included here primarily to avoid a secondary if statement within
                                      // the if block below - it will not be needed in any other situation
                }
            }

            if (current.element_pointer
                == iterator2.element_pointer)  // in case iterator2 was at beginning of it's group - also covers empty
                                               // range case (first == last)
            {
                return iterator2;
            }
            ...
            // Final group:
            // Code explanation:
            // If not erasing entire final group, 1. Destruct elements (if non-trivial destructor) and add locations to
            // group free list. 2. process skipfield. If erasing entire group, 1. Destruct elements (if non-trivial
            // destructor), 2. if no elements left in colony, clear() 3. otherwise reset end_iterator and remove group
            // from groups-with-erasures list (if free list of erasures present)
            ...
            {
                size_type number_of_group_erasures = 0;
                // Schema: first erased all non-erased elements until end of group & remove all skipblocks
                // post-iterator2 from the free_list. Then, either update preceding skipblock or create new one:
...
#endif
                {
                    while (current.element_pointer != iterator2.element_pointer) {
                        ...
                        else  // remove skipblock from group:
                        {
                            ...
                            if (next_free_list_index == std::numeric_limits<skipfield_type>::max()
                                && prev_free_list_index
                                       == std::numeric_limits<skipfield_type>::max())  // if this is the last skipblock
                                                                                       // in the free list
                            {
                                remove_from_groups_with_erasures_list(
                                    iterator2.group_pointer);  // remove group from list of free-list groups - will be
                                                               // added back in down below, but not worth optimizing for
                                ...
                                break;  // end overall while loop
                                     ...
                                     == std::numeric_limits<skipfield_type>::max())  // if this is the head of the free
                                                                                     // list
                            {
                ...
                const skipfield_type distance_to_iterator2
                    = static_cast<skipfield_type>(iterator2.element_pointer - current_saved.element_pointer);
                const skipfield_type index
                    = static_cast<skipfield_type>(current_saved.element_pointer - iterator2.group_pointer->elements);
                ...
                else  // If iterator 1 & 2 are in same group, but iterator 1 was not at start of group, and previous
                      // skipfield node is an end node in a skipblock:
                {
                ...
                iterator2.group_pointer->size
                    = static_cast<skipfield_type>(iterator2.group_pointer->size - number_of_group_erasures);
                total_size -= number_of_group_erasures;
            }
            else  // ie. full group erasure
            {
                ...
                if ((total_size -= current.group_pointer->size)
                    != 0)  // ie. either previous_group != NULL or next_group != NULL
                {
                    if (current.group_pointer->free_list_head != std::numeric_limits<skipfield_type>::max()) {
                        remove_from_groups_with_erasures_list(current.group_pointer);
                    }
...
// Destroy all elements if non-trivial:
      ...
      public:
        ...
        inline size_type size() const PLF_NOEXCEPT { return total_size; }
        ...
        inline size_type capacity() const PLF_NOEXCEPT { return total_capacity; }

        inline size_type memory() const PLF_NOEXCEPT
        {
            size_type mem = sizeof(*this);  // sizeof colony basic structure
            end_iterator.group_pointer->next_group
                = unused_groups_head;  // temporarily link the main groups and unused groups (reserved groups) in order
                                       // to only have one loop below instead of several

            for (group_pointer_type current = begin_iterator.group_pointer; current != NULL;
                 current = current->next_group) {
                mem += sizeof(group)
                       + (PLF_GROUP_ALIGNED_BLOCK_SIZE(current->capacity)
                          * sizeof(aligned_allocation_struct));  // add memory block sizes and the size of the group
                                                                 // structs themselves. The original calculation,
                                                                 // including divisor, is necessary in order to
                                                                 // correctly round up the number of allocations
            }
      ...
      private:
        // get all elements contiguous in memory and shrink to fit, remove erasures and erasure free lists. Invalidates
        // all iterators and pointers to elements.
        void consolidate()
        {
            ...
            if PLF_CONSTEXPR (std::is_move_constructible<element_type>::value
                ...
                colony temp(plf::colony_limits(tuple_allocator_pair.min_group_capacity,
                                               group_allocator_pair.max_group_capacity));
      ...
      public:
        void reshape(const plf::colony_limits capacities)
        {
            ...
            // Need to check all group sizes here, because splice might append smaller blocks to the end of a larger
            // block:
            for (group_pointer_type current = begin_iterator.group_pointer; current != end_iterator.group_pointer;
                 current = current->next_group) {
                if (current->capacity < tuple_allocator_pair.min_group_capacity
                    || current->capacity > group_allocator_pair.max_group_capacity) {
        ...
        {
            return plf::colony_limits(static_cast<size_t>(tuple_allocator_pair.min_group_capacity),
                                      static_cast<size_t>(group_allocator_pair.max_group_capacity));
        }
...
#    endif
        ...
        friend bool operator!=(const colony &lh, const colony &rh) { return !(lh == rh); }
        ...
        void reserve(size_type new_capacity)
        {
            ...
            skipfield_type remainder = static_cast<skipfield_type>(
                new_capacity - (number_of_max_groups * group_allocator_pair.max_group_capacity));
            ...
            if (begin_iterator.group_pointer == NULL)  // Most common scenario - empty colony
            {
                ...
                if (number_of_max_groups == 0) {
                    ...
                    first_unused_group = current_group
                        = allocate_new_group(group_allocator_pair.max_group_capacity, begin_iterator.group_pointer);
            ...
            while (number_of_max_groups != 0) {
                try {
                    current_group->next_group
                        = allocate_new_group(group_allocator_pair.max_group_capacity, current_group);
                }
                catch (...) {
      ...
      private:
        ...
        {
            if (total_size != 0)  // Necessary here to prevent a pointer matching to an empty colony with one memory
                                  // block retained with the skipfield wiped (see erase())
            {
                // Start with last group first, as will be the largest group in most cases:
                for (group_pointer_type current_group = end_iterator.group_pointer; current_group != NULL;
                    ...
                    if (reinterpret_cast<aligned_pointer_type>(element_pointer) >= current_group->elements
                        ...
                        return (*skipfield_pointer == 0)
                                   ? colony_iterator<is_const>(current_group,
                                                               reinterpret_cast<aligned_pointer_type>(element_pointer),
                                                               skipfield_pointer)
      ...
      public:
        ...
        inline allocator_type get_allocator() const PLF_NOEXCEPT { return *this; }

        void splice(colony &source) PLF_NOEXCEPT_SPLICE(allocator_type)
        {
            // Process: if there are unused memory spaces at the end of the current back group of the chain, convert
            // them to skipped elements and add the locations to the group's free list. Then link the destination's
            // groups to the source's groups and nullify the source. If the source has more unused memory spaces in the
            // back group than the destination, swap them before processing to reduce the number of locations added to a
            // free list and also subsequent jumps during iteration.
            ...
            // If there's more unused element locations in back memory block of destination than in back memory block of
            // source, swap with source to reduce number of skipped elements during iteration, and reduce size of
            // free-list:
      ...
      private:
        ...
        struct item_index_tuple {
            ...
            item_index_tuple(const pointer _item, const size_type _index) PLF_NOEXCEPT : original_location(_item),
      ...
      public:
        ...
        void sort(comparison_function compare)
        {
            ...
            tuple_pointer_type const sort_array
                = PLF_ALLOCATE(tuple_allocator_type, tuple_allocator_pair, total_size, NULL);
...
#else
                PLF_CONSTRUCT(tuple_allocator_type,
                              tuple_allocator_pair,
                              tuple_pointer,
...
// Now, sort the pointers by the values they point to (std::sort is default sort function if the macro below is not
// defined):
        ...
        struct colony_data : public uchar_allocator_type {
            ...
            unsigned char *
                *const bitfield_pointers;    // array of pointers to bitfields in the form of unsigned char arrays
            ...
            ~colony_data()
            {
                for (size_t index = 0; index != number_of_blocks; ++index) {
                    PLF_DEALLOCATE(
                        uchar_allocator_type, *this, bitfield_pointers[index], (block_capacities[index] + 7) / 8);
                }

                PLF_DEALLOCATE(uchar_allocator_type,
                               *this,
                               reinterpret_cast<unsigned char *>(block_pointers),
                               number_of_blocks * sizeof(aligned_pointer_type));
                PLF_DEALLOCATE(uchar_allocator_type,
                               *this,
                               reinterpret_cast<unsigned char *>(bitfield_pointers),
                               number_of_blocks * sizeof(unsigned char *));
                PLF_DEALLOCATE(uchar_allocator_type,
                               *this,
                               reinterpret_cast<unsigned char *>(block_capacities),
      ...
      private:
        void setup_data_cell(colony_data *data, const group_pointer_type current_group, const size_t capacity,
      ...
      public:
        colony_data *data()
        {
            ...
            setup_data_cell(
                data,
                end_iterator.group_pointer,
                static_cast<size_t>(end_iterator.group_pointer->last_endpoint - end_iterator.group_pointer->elements),
                group_number);
        ...
        void swap(colony &source) PLF_NOEXCEPT_SWAP(allocator_type)
        {
            ...
            if PLF_CONSTEXPR (std::is_trivial<group_pointer_type>::value && std::is_trivial<aligned_pointer_type>::value
                              && std::is_trivial<skipfield_pointer_type>::
                                  value)  // if all pointer types are trivial we can just copy using memcpy - avoids
                                          // constructors/destructors etc and is faster
            {
...
#    ifdef PLF_MOVE_SEMANTICS_SUPPORT  // Moving is probably going to be more efficient than copying, particularly if
                                       // pointer types are non-trivial:
...
#    endif
...
#endif
            {
                ...
                const skipfield_type swap_min_group_capacity = tuple_allocator_pair.min_group_capacity,
                                     swap_max_group_capacity = group_allocator_pair.max_group_capacity;
...
// Set up hive as alias of colony:
    ...
    template <class element_type, class allocator_type = std::allocator<element_type>,
              plf::hive_priority priority = plf::performance>
    ...
    struct colony_eq_to {
        ...
        explicit colony_eq_to(const element_type store_value)
            : /* may not be noexcept as the element may allocate and therefore potentially throw when copied */
              value(store_value)
        {
        }

        inline bool operator()(const element_type compare_value) const PLF_NOEXCEPT { return value == compare_value; }
    };
...
namespace std {
    ...
    inline void swap(plf::colony<element_type, allocator_type, priority> &a,
                     plf::colony<element_type, allocator_type, priority> &b) PLF_NOEXCEPT_SWAP(allocator_type)
    {
        a.swap(b);
    }

    template <class element_type, class allocator_type, plf::colony_priority priority, class predicate_function>
    typename plf::colony<element_type, allocator_type, priority>::size_type erase_if(
        plf::colony<element_type, allocator_type, priority> &container, predicate_function predicate)
    {
    ...
    template <class element_type, class allocator_type, plf::colony_priority priority>
    inline typename plf::colony<element_type, allocator_type, priority>::size_type erase(
        plf::colony<element_type, allocator_type, priority> &container, const element_type &value)
    {
